// Chinese Astrology Service Server
// Provides REST API endpoints for Chinese astrology calculations
...
try {
  ...
  logger = getLogger(null, 'chinese-service');
} catch (error) {
...
app.get('/api/status', (req, res) => {
  ...
  res.json({
    message: 'Chinese Astrology Service is operational',
    version: '1.0.0',
    features: [
...
const server = app.listen(PORT, () => {
  logger.info(`Chinese Astrology Service is running on port ${PORT}`);
...
process.on('SIGTERM', () => {
  ...
  server.close(() => {
    logger.info('Chinese Astrology Service terminated');
  });
});
